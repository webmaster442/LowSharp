    ...
    IDisposable
{
    ...
    public async Task Start()
    {
        ...
        {
            _dialogs.Error("Missing server", "The server executable can't be found");
            return;
        }
        ...
        {
            IsBusy = false;
            _dialogs.Error("Failed to start server", ex.Message);
        }
    }
    ...
    public async Task Connect()
    {
        ...
        {
            _dialogs.Error("Connection failed", ex.Message);
            return;
        }

        if (result.TryGetSuccess(out bool success) && !success)
        {
            _dialogs.Error("Connection failed", "Unknown error");
            return;
        }
    ...
    {
        if (_dialogs.Confirm("Confirm", "Invalidate server side cache?"))
        {
